...
interface DonorConfirmationNotification {
...
export class NotificationsService implements OnModuleInit {
  ...
  async sendDonorConfirmationNotification(
    notification: DonorConfirmationNotification,
  ) {
    const profile = await this.userProfileModel
      .where('_id', notification.userProfileId)
      .first();
    const pushToken = profile?.push_token;
    ...
    const scheduleText = new Intl.DateTimeFormat('id-ID', {
      ...
      hour: '2-digit',
      minute: '2-digit',
    }).format(notification.schedule);

    const message: ExpoPushMessage = {
      to: pushToken,
      sound: 'default',
      title: 'Pendaftaran donor dikonfirmasi',
      body: `${notification.hospitalName} menunggumu pada ${scheduleText}`,
      data: {
        type: 'donor_request_confirmed',
        route: '/(donor)/status',
        schedule: notification.schedule.toISOString(),
      },
    };
